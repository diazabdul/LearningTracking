...
public class Field : MonoBehaviour
{
    ...
    private List<Tiled> _connections = new List<Tiled>();
    private Tiled _connectionTile;
    ...
    void Start()
    {
        ...
        _grid = new Tiled[_dimensionX, _dimensionY];
        for (int y = 0; y < _dimensionX; y++)
        {
            ...
            {
                var tile = row.GetChild(x).GetComponent<Tiled>();
    ...
    void _CollectAmountToSolveFromTile(Tiled tile)
    {
        if (tile.cid > Tiled.UNPLAYABLE_INDEX)
        {
    ...
    void Update()
    {
        if (_canDrawConnection)
        {
            ...
            Tiled hoverTile = _grid[_mouseGridX, _mouseGridY];
            Tiled firstTile = _connections[0];
    ...
    bool _CheckIfTilesMatch(Tiled tile, Tiled another)
    {
    ...
    {
        textStatus.text = "Solve: " + solved + " from " + from;
    }
    ...
    Vector2 _FindTileCoordinates(Tiled tile)
    {
    ...
    private class Connection
    {
        ...
        public Connection(Tiled tile, Vector2 position)
        {
            this.tile = tile;
